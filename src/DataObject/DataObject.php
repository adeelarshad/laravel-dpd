...
namespace Proxi\Dpd\DataObject;
...
abstract class DataObject
{
    ...
    public function __construct($data)
    {
        ...
        foreach ($this->reflectionClass->getProperties() as $property) {
            ...
            if (isset($data[$propertyName])) {
                ...
                throw new \Proxi\Dpd\Exception\MissingPropertyException(
                    static::class,
                    $propertyName
                );
            }
    ...
    {
        return self::$annotationReader->getPropertyAnnotation($property, \Proxi\Dpd\DataObject\Annotation\Required::class);
    }

    private function isAccessible(\ReflectionProperty $property)
    {
        return self::$annotationReader->getPropertyAnnotation($property, \Proxi\Dpd\DataObject\Annotation\Accessible::class);
    }
     ...
     * @param \ReflectionProperty $property
     * @return string
     * @throws \Proxi\Dpd\Exception\UnkownTypeException
     */
    ...
    {
        if (!is_array($value)) {
            throw new \Proxi\Dpd\Exception\InvalidTypeException(
                static::class,
                $property->getName(),
        ...
        foreach ($value as $item) {
            if (
                ...
                && is_array($item)
                && is_subclass_of($itemType, \Proxi\Dpd\DataObject\DataObject::class)
            ) {
    ...
    private function setData($propertyName, $value)
    {
        ...
        if ($property && $this->isAccessible($property)) {
            ...
            if ($this->isArrayType($rawType)) {
                ...
                if ($propertyClass && is_array($value) && is_subclass_of($propertyClass, \Proxi\Dpd\DataObject\DataObject::class)) {
            ...
            throw new \Proxi\Dpd\Exception\InvalidPropertyException(
                static::class,
                $propertyName
            );
        }
                ...
                if (isset($value)) {
                    if (is_array($value)) {
                        foreach ($value as $childKey => $childValue) {
                            if (is_subclass_of($childValue, \Proxi\Dpd\DataObject\DataObject::class)) {
                                $value[$childKey] = $childValue->toDataArray();
                            }
                        }
                    }
                    $data[$property->getName()] =
                        is_subclass_of($value, \Proxi\Dpd\DataObject\DataObject::class) ? $value->toDataArray() : $value;
                }
            }
        }
